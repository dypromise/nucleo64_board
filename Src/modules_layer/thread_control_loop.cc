...
void thread_control_loop_init(void) {
    ...
    if (NULL == cmd_queue) {
        MM_DEBUG_ERROR("Creating cmd queue failed!!");
    }
    ...
    if (result != pdPASS) {
        MM_DEBUG_ERROR("Creating motion task failed!!");
    }
}
...
static void control_loop(void *pvParameters) {
    ...
    while (1) {
        ...
        if (state.range && (range.left > (MEASURE_RANGE_L_MIN_DETECT))
            ...
            feedback_R =
                    (range.right - range.left - MEASURE_RANGE_R_OFFSET) / 10;
        } else if (state.range &&
                   range.left > (MEASURE_RANGE_L_MIN_DETECT + 50)) {
            ...
            feedback_R = (MEASURE_RANGE_R_M_DETECT - range.left) / 10;
                   ...
                   range.right > (MEASURE_RANGE_R_MIN_DETECT + 50)) {
        ...
        if (!state.cmd_ready) {
            ...
            if ((state.left_wheel == WHEEL_DISABLED) &&
                (state.right_wheel == WHEEL_DISABLED)) {
                state.cmd_ready = 1;
                // state.current_cmd = CMD_NOTHING;
                if ((state.current_cmd == CMD_R) ||
                    (state.current_cmd == CMD_L)) {
                    pid_reset(&pid.tran);
                    pid_reset(&pid.rot);
                }
                // Send a notification
