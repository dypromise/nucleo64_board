...
MouseController::MouseController(char *filename, IOInterface *fileIO,
                                 IOInterface *printIO, FinderInterface *finder,
...
bool MouseController::anyDestinationCellSearched() {
    ...
    for (i = 0; i < destinations.size(); i++) {
        if (getDis(destinations[i].row, destinations[i].col) !=
            CELL_DISTANCE_UNREACHED) {
...
void MouseController::getDistanceAllCell(void) {
    int currentPathDistance =
            CELL_DISTANCE_START + 1;  //This is how far the 'water' has flowed
    int row;
    int col;
    ...
    while (1) {
        ...
        if (!isFound) {
            ...
            do {
                ...
                isAdjacent =
                        ((posFromPath.getNextPos(row_plus) ==
                          posFromAvailable.getCurrentPos())
                         || (posFromPath.getNextPos(col_plus) ==
                             posFromAvailable.getCurrentPos())
                         || (posFromPath.getNextPos(row_minus) ==
                             posFromAvailable.getCurrentPos())
                         || (posFromPath.getNextPos(col_minus) ==
                             posFromAvailable.getCurrentPos())
                        );
                isNextDistance =
                        (getDis(posFromPath) + 1) == getDis(posFromAvailable);
...
void MouseController::moveNextShortestCell() {
    /* 1. turn first */
    ...
    if (isInDestinationCell()) {
        destinations.erase(std::remove(destinations.begin(),
                                       destinations.end(), getCurrentPos()),
                           destinations.end());
    }
