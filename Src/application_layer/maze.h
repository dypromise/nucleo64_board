...
private:
    /* walls in y-direction (or row-increasing) */
    Wall rowWall[CONFIG_MAX_ROW_SIZE + 1][CONFIG_MAX_COL_SIZE];

    /* walls in x-direction (or column-increasing)*/
    Wall colWall[CONFIG_MAX_ROW_SIZE][CONFIG_MAX_COL_SIZE + 1];

    /* each cells in the maze */
    Cell cell[CONFIG_MAX_ROW_SIZE][CONFIG_MAX_COL_SIZE];
